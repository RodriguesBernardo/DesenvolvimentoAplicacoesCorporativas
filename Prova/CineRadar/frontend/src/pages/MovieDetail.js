...
const MovieDetail = () => {
  ...
  // Check watchlist status when movie is loaded
  useEffect(() => {
    const checkWatchlistStatus = async () => {
      ...
      try {
        ...
        // Define como false para evitar problemas na UI
        setInWatchlist(false);
        // Mostra mensagem apenas se não for erro 401 (não autorizado)
    ...
    <div className="movie-detail-page">
          ...
          <Row className="align-items-end">
            ...
            <Col lg={8} className="hero-content">
              <div className="d-flex flex-wrap align-items-center gap-3 mb-3">
                ...
                {movie.adult && <Badge bg="danger">18+</Badge>}
              </div>
      ...
      <Container className="py-5">
        ...
        <Row className="mb-5 g-4">
          <Col lg={8}>
            <MovieInfo movie={movie} credits={credits} formatMoney={formatMoney} formatRuntime={formatRuntime}/>
          </Col>
          <Col lg={4}>
  ...
  <Card className="shadow-sm mb-4">
    <Card.Body className="p-3">
      <div className="d-flex flex-wrap align-items-center gap-4">
        ...
        {providers.flatrate && <ProviderGroup name="Streaming" providers={providers.flatrate.slice(0, 3)} />}
        {providers.buy && <ProviderGroup name="Comprar" providers={providers.buy.slice(0, 2)} />}
        {providers.rent && <ProviderGroup name="Alugar" providers={providers.rent.slice(0, 2)} />}
      </div>
    </Card.Body>
  </Card>
);

const ProviderGroup = ({ name, providers }) => (
  <div className="d-flex align-items-center">
    <span className="me-2 text-muted small">{name}:</span>
...
const MovieInfo = ({ movie, credits, formatMoney, formatRuntime }) => (
  <Card className="shadow-sm">
    <Card.Body className="p-3">
      <Row>
        <Col md={6}>
          <InfoItem label="Diretor" value={credits.crew?.find(p => p.job === 'Director')?.name || 'Desconhecido'} />
          <InfoItem label="Lançamento" value={new Date(movie.release_date).toLocaleDateString('pt-BR')} />
          <InfoItem label="Status" value={movie.status} />
        </Col>
        <Col md={6}>
          <InfoItem label="Orçamento" value={formatMoney(movie.budget)} />
          <InfoItem label="Bilheteria" value={formatMoney(movie.revenue)} />
          <InfoItem label="Duração" value={formatRuntime(movie.runtime)} />
        </Col>
      </Row>
    </Card.Body>
  </Card>
...
const MediaTab = ({ selectedTrailer, videos, onTrailerSelect, onShowTrailer }) => (
  <div className="mt-4">
    {selectedTrailer && (
...
const TrailerModal = ({ show, onHide, trailer, movieTitle }) => (
  <Modal 
    show={show} 
    onHide={onHide}
    size="xl"
  ...
  >
    <Modal.Header closeButton closeVariant="white" className="border-secondary">
      <Modal.Title>
        {trailer?.name || `Trailer: ${movieTitle}`}
      </Modal.Title>
    </Modal.Header>
      ...
      {trailer ? (
        <div className="ratio ratio-16x9">
          {trailer.site === 'YouTube' ? (
            ...
            <iframe
              src={`https://player.vimeo.com/video/${trailer.key}?autoplay=1`}
              title={trailer.name}
